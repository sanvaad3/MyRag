...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`✅ Uploaded: ${file.name} (${chunks.length} chunks with embeddings)`)
    console.log(`📝 First chunk preview: ${chunks[0]?.content.substring(0, 200)}...`)
    console.log(`📊 Total documents in store: ${documentStore.count()}`)
...
export async function GET() {
  try {
    ...
    console.log(`📚 Document store status: ${documents.length} documents, ${allChunks.length} total chunks`)
